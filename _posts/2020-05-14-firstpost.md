...
<p> $\underset{u}{\text{min}}\big [x^T Q x + u^T R u + \frac{\partial J^*}{\partial x}(Ax + Bu) + \frac{\partial J^*}{\partial t} \big ] $</p>

With a quadratic form on $u$, there is an optimum such that 
<p> $\frac{\partial}{\partial u} (\text{everything inside of the min argument of the HJB}) = 0 $</p>
